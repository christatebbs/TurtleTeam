...
public enum FirstBattleState { START, PLAYERTURN, FIRSTENEMYTURN, SECONDENEMYTURN, THIRDENEMYTURN, WON, LOST, PLAYERACTION, ENEMYSELECTHIT, 
ENEMYSELECTSKILL } 
...
public class FirstBattle: MonoBehaviour {
	...
	IEnumerator SetupBattle(){
		...
		GameObject enemyThree = (GameObject)Instantiate (Resources.Load("Wrapuchin"), enemyBattleStation3);
		...
		dialogueText.text = "Three Wrapuchins appear in front of you!"; 
		...
		if (playerLevel.stats [4] >= enemyUnitOne.stats [4]) {
			...
			state = FirstBattleState.PLAYERTURN;
			PlayerTurn (); 
		} else { 
			...
			state = FirstBattleState.FIRSTENEMYTURN;
		...
		if (randomNum == 1) {
			...
			yield return new WaitForSeconds (2f); 
				if (!enemyUnitTwo.isKilled) { 
					state = FirstBattleState.SECONDENEMYTURN; 
					StartCoroutine (SecondEnemyTurn ()); 
					yield break;
				} else if (!enemyUnitThree.isKilled) { 
					state = FirstBattleState.THIRDENEMYTURN; 
	...
	IEnumerator ThirdEnemyTurn(){
		...
		dialogueText.text = "Third Wrapuchin's turn!"; 
		yield return new WaitForSeconds(2f); 
	...
	IEnumerator EndBattle(){ 
		...
		} else if (state == FirstBattleState.LOST) { 
	...
	public void OnAttackButton(){
		...
		if (state != FirstBattleState.PLAYERTURN) { 
			return; 
		}
		state = FirstBattleState.ENEMYSELECTSKILL;
	...
	public void OnHitButton(){
		if (state == FirstBattleState.PLAYERTURN) { 
			...
			state = FirstBattleState.ENEMYSELECTHIT;
		}
	}
	...
	IEnumerator Hit(String enemyToHit){ 
		state = FirstBattleState.PLAYERACTION;
		bool doesHit;
